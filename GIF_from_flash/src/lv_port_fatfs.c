...
void lv_fs_if_init(void)
{
	...
	bool drv = lv_fs_is_ready('S');
	if(drv == NULL)
	{

		LV_LOG_WARN("fs register error");

	}else{

		LV_LOG_WARN("fs register OK");
...
/*
   ...
   uint32_t Z, resCount;
	...
	if (res == LV_FS_RES_OK) {
		fprintf(stdout,"Create file 'hello.txt' Success res : %d",res);
		res = lv_fs_write(&f,"hellobug esp32", sizeof("hellobug esp32"), &Z);// 创建一个文件
		if (res == LV_FS_RES_OK) {
			fprintf(stdout,"Write file 'hello.txt' Success res : %d WriteByte: %d",res,resCount);
		}		
	}
*/
}
...
static void fs_init(void)
{
	...
	FRESULT fres = FR_NOT_READY;
  	...
  	if (fres != FR_OK) {
    	fres = f_mount(fs, "0:", 1);
    	if (fres != FR_OK) {
      		LV_LOG_WARN("SD Card mounted error. (%d)\n", fres);
    	} else
	    	LV_LOG_WARN("SD Card mounted successfully.\n\n");
  	}
