...
void gauge_win_create(lv_obj_t * parent)
{
	...
	lv_gauge_set_range(gauge1,0,180);//设置仪表盘的范围
	...
	lv_gauge_set_value(gauge1,0,TMethodSpeed);//设置指针1指向的数值,我们把指针1当作速度指针
	lv_gauge_set_value(gauge1,1,150);//设置指针2指向的数值,就让它指向关键数值点
	lv_gauge_set_critical_value(gauge1,150);//设置关键数值点
	...
	lv_obj_set_width(label1,90);//设置宽度
	...
	lv_label_set_text(label1,"0 r/m");//设置文本
	lv_label_set_recolor(label1,true);//使能文本重绘色
	
	//创建一个刻度指示器
	lmeter1 = lv_lmeter_create(win,NULL);
	lv_obj_set_size(lmeter1,150,160);//设置大小
	lv_obj_align(lmeter1,gauge1,LV_ALIGN_OUT_BOTTOM_MID,0,10);//设置对齐方式
	lv_lmeter_set_range(lmeter1,0,170);//设置范围
	lv_lmeter_set_value(lmeter1,TMethodSpeed);//设置当前的值
